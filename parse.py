...
def parse_template(text: str) -> tuple:
    ...
    for i in range(0, len(sections) - 1, 2):
        ...
        if file_match:
            data["file"] = file_match.group("filename").strip()
            files.append(data)
        elif output_match:
...
#     Returns:
#     - tuple: A tuple containing a list of dictionaries with parsed data and potential errors.
#     """
...
#     # Check if at least one item has a 'file' key
#     if not any("file" in item for item in parsed_data):
#         errors.append("Error: At least one section must have 'file' metadata.")
